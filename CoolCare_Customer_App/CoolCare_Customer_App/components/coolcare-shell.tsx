...
const navItems = [
  { label: 'Dashboard', shortLabel: 'Dashboard', href: '/', icon: Gauge },
  { label: 'Book Service', shortLabel: 'Book', href: '/book', icon: CalendarDays },
  { label: 'My Bookings', shortLabel: 'Bookings', href: '/bookings', icon: ClipboardCheck },
  { label: 'Maintenance History', shortLabel: 'History', href: '/history', icon: History },
];
    ...
    <div className="min-h-screen bg-background text-foreground">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[280px] border-r border-border bg-white px-5 py-7 lg:flex lg:flex-col">
        ...
        <div className="mt-auto rounded-2xl bg-[linear-gradient(145deg,#eff5ff,#ecfffb)] p-5">
          ...
          <p className="font-semibold">Annual Care Plan</p>
          <p className="mt-1 text-sm leading-5 text-muted-foreground">2 maintenance visits remaining this year.</p>
        </div>
      </aside>
      ...
      <main className="pb-24 lg:ml-[280px] lg:pb-10">
        <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-border/80 bg-white/90 px-5 backdrop-blur-xl sm:px-8 lg:px-10">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden text-right sm:block"><p className="text-sm font-semibold">Alice Tan</p><p className="text-xs text-muted-foreground">Demonstration customer</p></div>
            <div className="grid size-10 place-items-center rounded-full bg-secondary/15 text-sm font-bold text-secondary">AT</div>
          </div>
        </header>
        {children}
      </main>
...
function Brand({ compact = false }: { compact?: boolean }) {
  return (
    <Link href="/" className={compact ? 'flex items-center gap-2.5' : 'mb-9 flex items-center gap-3 px-2'}>
