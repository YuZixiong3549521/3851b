...
export async function createBooking(pool, untrustedInput) {
  ...
  try {
    ...
    const customer = await getDemoCustomer(connection);
    ...
    const totalAmount = Number(serviceRows[0].basePrice) * uniqueUnitIds.length;
...
export async function listBookings(pool, scope = 'all') {
...
export async function getBookingReport(pool, bookingId) {
  const customer = await getDemoCustomer(pool);
  const [rows] = await pool.execute(
    `SELECT
       b.booking_id AS bookingId,
