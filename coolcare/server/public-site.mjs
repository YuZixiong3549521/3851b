...
import { lockCustomer, normalizeAddress, assertAddressBookingLimit, resolveBookingSelection, saveBookingSelection, restoreMembershipVisit, attachBookingSelections } from './customer/booking-options.mjs';
...
export async function createPublicBooking(pool,user,input,{legacy=false}={}) {
 const data=z.object({expectedUserId:z.coerce.number().int().positive().optional(),serviceType:z.string().min(1).max(120).optional(),serviceIds:z.array(z.coerce.number().int().positive()).min(1).max(10).optional(),packageId:z.coerce.number().int().positive().optional(),subscriptionId:z.coerce.number().int().positive().optional(),servicePackage:z.string().max(120).optional(),numberOfUnits:z.coerce.number().int().min(1).max(10),preferredDate:legacy?z.string():date,timeWindow:legacy?z.string():z.enum(times),serviceAddress:z.string().trim().min(5).max(255),phone:z.string().max(30).optional(),symptoms:z.string().max(1000).optional(),specialNotes:z.string().max(1000).optional(),requestId:z.uuid().optional()}).parse(input);
 ...
 try {
  ...
  if(data.requestId){const [[existing]]=await conn.execute('SELECT b.booking_id,b.total_amount,b.booking_status FROM web_booking_details d JOIN booking b ON b.booking_id=d.booking_id WHERE d.request_id=? AND b.customer_id=?',[data.requestId,customer.customerId]);if(existing){const emailNotification=legacy?undefined:await enqueueBookingEmail(conn,existing.booking_id);await conn.commit();return {id:existing.booking_id,totalAmount:existing.total_amount,status:existing.booking_status,emailNotification};}}
  const selection=await resolveBookingSelection(conn,customer.customerId,data,data.numberOfUnits);
  if(!legacy)await assertAddressBookingLimit(conn,customer.customerId,data.serviceAddress,data.preferredDate);
  ...
  const total=selection.totalAmount;
  const [booking]=await conn.execute("INSERT INTO booking(customer_id,address_id,service_id,subscription_id,preferred_service_date,preferred_time_slot,problem_description,booking_status,total_amount) VALUES (?,?,?,?,?,?,?,'Submitted',?)",[customer.customerId,addressId,selection.services[0].serviceId,selection.subscription?.subscriptionId??null,data.preferredDate,data.timeWindow,data.symptoms||null,total]);
  ...
  await conn.execute('INSERT INTO web_booking_details(booking_id,service_package,contact_phone,special_notes,request_id) VALUES (?,?,?,?,?)',[booking.insertId,selection.package?.name||data.servicePackage||selection.serviceName.slice(0,120),data.phone||user.phone||null,data.specialNotes||null,data.requestId||null]);
  await saveBookingSelection(conn,booking.insertId,selection);
  ...
  const emailNotification=legacy?undefined:await enqueueBookingEmail(conn,booking.insertId);
  await conn.commit();return {id:booking.insertId,totalAmount:total,status:'Submitted',serviceName:selection.serviceName,services:selection.services,package:selection.package??null,emailNotification};
