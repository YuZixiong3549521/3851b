...
export function AnnualBookingSummary({ firstDate, timeSlot, totalAmount, saved, compact = false }: {
  ...
  return <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-left text-sm">
    ...
    {!compact && <p className="mt-3 text-xs leading-5 text-muted-foreground">Choose a weekday at least 14 calendar days ahead in Singapore time for the first visit. Later quarterly dates falling on a weekend move to the following Monday. Dates and time windows are requests, subject to availability; submitting does not assign a technician or take payment.</p>}
  </div>;
}
