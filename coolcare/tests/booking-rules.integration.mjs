...
const body=(overrides={})=>({serviceType:'Air Conditioning Cleaning',numberOfUnits:2,preferredDate:date(0),timeWindow:'09:00 AM - 11:00 AM',serviceAddress:'42 Rules Test Avenue #02-10',requestId:randomUUID(),...overrides});
...
test('both booking routes enforce normalized address limits, cancellation and rescheduling with real MySQL',async()=>rollbackFixture(async(db,c,user)=>{
  ...
  const [[service]]=await c.execute("SELECT service_id FROM service_catalog WHERE service_name='Air Conditioning Cleaning'");
...
test('multi-service and bundle prices are authoritative snapshots and units must belong to selected address',async()=>rollbackFixture(async(db,c,user)=>{
  const options=await getBookingOptions(db,user.customerId);
  assert.ok(options.services.length>=4);assert.ok(options.bundles.length>=2);
  const selected=options.services.slice(0,2);
  const result=await createPublicBooking(db,user,body({serviceType:undefined,serviceIds:selected.map(s=>s.serviceId),totalAmount:1}));
  assert.equal(result.totalAmount,selected.reduce((sum,s)=>sum+s.basePrice+s.additionalUnitPrice,0));
  const [[snapshot]]=await c.execute('SELECT COUNT(*) AS n,SUM(line_total) AS total FROM booking_service WHERE booking_id=?',[result.id]);
  assert.equal(snapshot.n,2);assert.equal(Number(snapshot.total),result.totalAmount);
  const list=await listBookings(db,'all',user.id);
  assert.equal(list[0].services.length,2);assert.ok(list[0].serviceName.includes(' + '));
  const bundle=options.bundles[0];
  const bundled=await createPublicBooking(db,user,body({packageId:bundle.packageId,serviceType:undefined,serviceIds:bundle.serviceIds,preferredDate:date(8),numberOfUnits:3}));
  assert.equal(bundled.totalAmount,bundle.price+2*bundle.additionalUnitPrice);
  const [[packageRow]]=await c.execute('SELECT package_name FROM booking_package WHERE booking_id=?',[bundled.id]);
  assert.equal(packageRow.package_name,bundle.name);
  await assert.rejects(createPublicBooking(db,user,body({packageId:bundle.packageId,serviceIds:[99999999],preferredDate:date(20)})),error=>error.status===400);
  ...
  await assert.rejects(createBooking(db,{serviceIds:[selected[0].serviceId],addressId:address.address_id,unitIds:[differentUnit.insertId],preferredDate:date(30),timeSlot:'09:00 - 11:00'},user.id),error=>error.status===400);
  await assert.rejects(createBooking(db,{serviceIds:[selected[0].serviceId],addressId:address.address_id,unitIds:[differentUnit.insertId],preferredDate:'2028-02-30',timeSlot:'09:00 - 11:00'},user.id),error=>error.status===400);
}));

test('membership visit ownership, reservation, retry and cancellation are transactional',async()=>rollbackFixture(async(db,c,otherUser)=>{
  const [[subscription]]=await c.query(`SELECT cs.subscription_id,cs.customer_id,c.user_id FROM customer_subscription cs JOIN customer c ON c.customer_id=cs.customer_id ORDER BY cs.subscription_id LIMIT 1`);
  await c.execute("UPDATE customer_subscription SET start_date=?,end_date=?,remaining_service_count=1,subscription_status='Active' WHERE subscription_id=?",[date(-1),date(180),subscription.subscription_id]);
  const user={id:subscription.user_id};
  const request=body({subscriptionId:subscription.subscription_id,serviceType:undefined,serviceAddress:'Reserved Membership Test '+randomUUID()});
  await assert.rejects(createPublicBooking(db,otherUser,request),error=>error.status===403);
  const result=await createPublicBooking(db,user,request);
  assert.equal(result.totalAmount,0);
  assert.equal((await createPublicBooking(db,user,request)).id,result.id);
  let [[balance]]=await c.execute('SELECT remaining_service_count FROM customer_subscription WHERE subscription_id=?',[subscription.subscription_id]);
  assert.equal(balance.remaining_service_count,0);
  await assert.rejects(createPublicBooking(db,user,{...request,requestId:randomUUID(),preferredDate:date(10)}),error=>error.status===409);
  await assert.rejects(changePublicBooking(db,user,result.id,'reschedule',{preferredDate:date(181),timeWindow:request.timeWindow}),error=>error.status===409);
  await changePublicBooking(db,user,result.id,'cancel',{status:'Cancelled'});
  [[balance]]=await c.execute('SELECT remaining_service_count FROM customer_subscription WHERE subscription_id=?',[subscription.subscription_id]);
  assert.equal(balance.remaining_service_count,1);
  await assert.rejects(changePublicBooking(db,user,result.id,'cancel',{status:'Cancelled'}),error=>error.status===409);
  assert.ok((await createPublicBooking(db,user,{...request,requestId:randomUUID(),preferredDate:date(10)})).id);
  const [[mail]]=await c.execute('SELECT COUNT(*) AS n FROM booking_email_outbox WHERE booking_id=?',[result.id]);
...
test('public and customer requests wait for the same customer lock before reading booking limits',async()=>{
  const [[fixture]]=await pool.query(`SELECT c.user_id,c.customer_id,sa.address_id,au.unit_id,sc.service_id
    ...
    CROSS JOIN service_catalog sc WHERE sc.service_name='Air Conditioning Cleaning' ORDER BY c.customer_id LIMIT 1`);
