...
for (const migration of ['06-booking-packages.sql','07-inventory-audit.sql','08-booking-email.sql']) sql += '\n' + readFileSync(resolve(root,'database',migration),'utf8') + '\n';
...
for (const table of ['booking_service','booking_package','booking_email_outbox','inventory_transaction_revision']) sql+=`GRANT INSERT ON coolcare_service_app.${table} TO '${user}'@'%';\n`;
