export const statuses=['Assigned','On the Way','In Progress','Pending','Completed','Cancelled'];
...
export const formatTime=time=>{if(!time)return 'Not scheduled';const [h,m]=time.split(':');return `${String(Number(h)%12||12).padStart(2,'0')}:${m} ${Number(h)<12?'AM':'PM'}`;};
export const isUpcoming=(job,today)=>job.date>today&&!['Completed','Cancelled'].includes(job.status);
