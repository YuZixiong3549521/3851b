...
function Report({report}) {
 return <div className="tech-report">
  <p><strong>Work performed:</strong> {report.workPerformed}</p>
...
function StockOut({jobId,options,onSaved,onReload,lock,externallyLocked}) {
 ...
 const qty=Number(quantity),excess=part&&Number(part.issued)+qty>part.recommended;
 ...
 return <form onSubmit={issue} className="tech-stock-form"><fieldset disabled={externallyLocked&&!busy&&!uncertain}>
  ...
  {part&&<div className="tech-guidance"><p>Per AC: {part.recommended_units_per_ac} {part.stock_unit}. Recommended total: {part.recommended}. Already issued: {part.issued}.</p><p>{part.usage_note||'Review the actual parts required on site.'}</p></div>}
  <label>Quantity<Input type="number" min="1" max={part?.current_stock||2147483647} step="1" required value={quantity} disabled={busy||uncertain} onChange={e=>setQuantity(e.target.value)}/></label>
  <label>Description<Textarea maxLength={500} value={remarks} disabled={busy||uncertain} onChange={e=>setRemarks(e.target.value)} placeholder="Describe what these parts will be used for."/></label>
  {excess&&<label className="tech-excess"><input type="checkbox" checked={ack} disabled={busy||uncertain} onChange={e=>setAck(e.target.checked)}/><span>This exceeds the work order recommendation. I have reviewed the extra quantity and acknowledge the additional usage.</span></label>}
  {part&&qty>part.current_stock&&<p className="tech-error" role="alert">Not enough stock. Only {part.current_stock} {part.stock_unit} available.</p>}
...
export default function JobDrawer({job:summary,onClose}) {
 ...
 useEffect(()=>{
  if(useMock)return;
  const controller=new AbortController();setDetail(null);setOptions(null);setError('');
  Promise.all([technicianRequest(`/jobs/${summary.jobId}`,{signal:controller.signal}),technicianRequest(`/jobs/${summary.jobId}/parts`,{signal:controller.signal})]).then(([details,parts])=>{if(!controller.signal.aborted){setDetail(details);setOptions(parts);}}).catch(e=>{if(!controller.signal.aborted)setError(e.message);});
  return()=>controller.abort();
 },[summary.jobId,version]);
 const job=detail?.job||summary;
 return <Dialog open onOpenChange={open=>{if(!open&&!locked)onClose();}}><DialogContent showCloseButton={false} className="max-h-[90vh] overflow-y-auto sm:max-w-2xl p-0" aria-describedby={undefined}><div className="drawer-inner">
  <div className="drawer-heading"><span className="job-id">{job.id}</span><Button variant="ghost" autoFocus className="icon-button" disabled={locked} onClick={onClose} aria-label="Close job details"><X/></Button></div>
  <DialogTitle>{job.customer}</DialogTitle><div className="drawer-badges"><StatusBadge value={job.status}/><PriorityBadge value={job.priority}/></div>
  <h3>Appointment</h3><p className="cell-icon"><CalendarDays size={18}/>{formatDate(job.date)}</p><p className="cell-icon"><Clock3 size={18}/>{formatTime(job.time)}</p><p className="cell-icon"><Wrench size={18}/>{job.serviceType}</p>
  <p>Estimated duration: {job.estimatedDurationMinutes?`${job.estimatedDurationMinutes} minutes`:'Not recorded'}</p><p>Last modified: {job.lastModified||'Not recorded'}</p>
  <h3>Service address</h3><p className="cell-icon"><MapPin size={18}/>{job.address}</p><p>{job.acCount??'—'} AC unit(s) linked to this booking.</p>
  <h3>Reported problem / service notes</h3><p>{job.reportedProblem||'No additional details provided.'}</p>
