...
当前 **66 项测试**通过（32 项主应用、4 项技师、30 项真实 MySQL 集成测试），TypeScript 和两套生产构建通过。覆盖客户与记录归属、CSRF、事务、幂等重试、地址保留与默认/归档、账号切换保护、预约限制、年度套餐、库存审计、时间戳时区及私有照片 HTTP 权限。数据库测试通过事务回滚清理，自增编号可能出现空号。
