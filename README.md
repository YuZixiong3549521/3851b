...
49 项自动化测试通过，覆盖注册登录、角色及记录归属、地址保存与去重、数量式及原设备式预约、最近预约排序、14 天提前量、新加坡跨日与周末限制、四次年度预约与邮件的原子写入、重复请求、季度内改期、取消、库存审计及技师清洗评估。TypeScript 和两套生产构建通过。数据库测试采用事务回滚，自增编号可能出现空号。

地址流程的浏览器验证使用无设备的新客户，真实保存并重读地址，提交年度四单和另一地址的单次预约，独立 SQL 核对金额、数量和设备关联，并验证 Dashboard 取最近预约及 WebMCP 拒绝后可修正重试。桌面和 390px 布局通过检查，独立 QA 账号、地址和订单已清理。
